...
;; Time-stamp: <2011-10-05 08:59:01EDT types.lisp>
;;
...
(export '(*cstd-cl-type-mapping* floating-point-association all-types
...
(defparameter *fp-type-mapping*
  '((:float . single-float) (:double . double-float)
    ...
    ((:struct complex-float-c) . (complex single-float))
    ((:struct complex-double-c) . (complex double-float))
    ;; For those implementations that support a separate long-double
    ;; type assume this mapping:
    #+long-double
