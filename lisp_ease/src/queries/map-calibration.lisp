; Contains useful functions which help to find out the offset between the OpenEase (VR) and the bullet world semantic map.
; The scenario is, that an episode is recorded, in which the human places cereal objects on the edges
; of the kitchen island table, and milk objects on the edges of the sink table, so that one can differentiate,
; between the two tables, and can see how offset they are in the bullet world map.
; Then the transform can be adjusted and the objects can be moved, until they are on the edges of the tables in the bullet world as well.
