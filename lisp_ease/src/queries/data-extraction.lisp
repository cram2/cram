...
"Queries OpenEase for the episode data. This can take a while. Saves the result into the *orig-poses-list* and the *poses-list* variables.
...
;; TODO use object type or something instead of just the count... Or a combination of both? or with time stamps?
(defun get-next-obj-poses (count)
  "Extracts the data of a specific event within the current episode. The event can be currently specifies with the count parameter. Also sets the *poses-list* variable to the new event data.
...
(defun make-poses (name &optional (poses-list *poses-list*))
  "Calls make-pose to convert the list of values from poses-list to a cl-tf:transform, and then applies all the adjustments needed to convert the pose from the OpenEase map into a proper pose for the bullet world.
NAME: The name of an item in the *poses-list* of which the pose is needed. The time stamp is included into the name, meaning possible parameters could be: 
OPTIONAL POSES-LIST: A different poses-list can be given if needed. Otherwise *poses-list* will be used as default.
RETURNS: A cl-tf:transform of the given object at the given timestamp, With all the adjustments for the differences between the OpenEase and the bullet world already beeing made."
...
(defun make-pose (pose)
  "Makes a proper cl-tf:transform with a 3d-vector for translation and a quaternion for rotation.
