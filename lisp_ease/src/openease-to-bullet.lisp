; This file contains all functions needed to adjust the poses and transforms from the OpenEase semantic map
; to the bullet world semantic map, since the kitches are slightly different and therefore the poses
; will be off without the usage of these functions.
; Also there are variations within the data representation of OpenEase and Bulelt World,
; these functions also take care of that. 
(in-package :le)
...
(defun human-to-robot-hand-transform ()
  "Defines the offset between the human hand from the virtual reality to the robot standart gripper, which has been calculated manually.
...
(defun place-pose-btr-island ()
  "Calculates the placing pose for an object, relative to the bullet world kitchen island. This is needed, since the OpenEase kitchen island and the bullet world kitchen island, are slightly offset to one another, and the offset fixing the general semantic map offset, is not enough to fix it. Therefore relative poses are being calculated.
RETURNS: A cl-tf:transform representing the pose and orientation, at which the robot in the bullet world should place the object currently in hand."
  (let* (table-pose-oe
         table-pose-bullet
         place-pose)
