...
%add_subaction_with_task(Action,SubAction,TaskType) :- execution_agent(Agent), tell([has_type(Role, soma:'AgentRole'), has_role(Agent,Role), is_action(SubAction), has_type(Task,TaskType), executes_task(SubAction,Task), has_subevent(Action,SubAction),task_role(Task, Role),is_performed_by(SubAction,Agent)]),notify_synchronize(event(Event)), !.
...
%add_participant_with_role(Action, ObjectId, RoleType) :- ask(executes_task(Action, Task)), tell([has_participant(Action,ObjectId), has_type(Role, RoleType), has_role(ObjectId,Role) during [0.0,0.0],task_role(Task, Role)]).
