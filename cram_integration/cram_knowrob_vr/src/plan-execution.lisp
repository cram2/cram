...
RETURNS: "
  (pick-and-place
   (set-grasp-base-pose (umap-T-laser-T-human type))
   (set-grasp-look-pose (umap-T-robot type))
   (set-grasp-base-pose (umap-T-laser-T-place type :end))
   (set-grasp-look-pose (place-pose type))
   (set-grasp-look-pose (place-pose type))
   type))
...
grasp the object or in order to place it."
  (ecase event-time
    (:start
     (move-to-object
      (set-grasp-base-pose (umap-T-human type))
      (set-grasp-look-pose (umap-T-robot type))))
    (:end
     (move-to-object
      (set-grasp-base-pose (ucamera-T-usurface type :end))
      (set-grasp-look-pose (place-pose type))))))
